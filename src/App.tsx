...
import { z } from 'zod'
import { Archive, ArrowDownToLine, ArrowLeftRight, Check, ChevronDown, CircleMinus, CornerDownLeft, Download, FileImage, ImagePlus, Maximize2, MoreHorizontal, PanelLeftClose, PanelLeftOpen, Play, Plus, RefreshCw, Trash2, UploadCloud, X, Zap } from 'lucide-react'
...
export default function App() {
  ...
  const onDrop = (event: React.DragEvent) => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files) }
  return <div className={`app ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    <aside className="sidebar">
      <div className="sidebar-header"><div className="brand"><div className="brand-mark"><Zap size={16} fill="currentColor" /></div><span className="brand-name">imageflow</span><span className="version">BETA</span></div><button className="sidebar-toggle" aria-label={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'} onClick={() => setSidebarCollapsed(value => !value)}>{sidebarCollapsed ? <PanelLeftOpen size={16}/> : <PanelLeftClose size={16}/>}</button></div>
      <div className="sidebar-content"><div className="sidebar-group"><div className="side-label">WORKSPACE</div><button className="nav-item active"><FileImage size={17}/> <span className="nav-text">Image workflow</span> <span className="nav-count">{items.length || ''}</span></button><button className="nav-item muted"><Archive size={17}/> <span className="nav-text">Exports</span></button></div></div>
      <div className="sidebar-footer"><div className="privacy"><span className="status-dot"/><div className="nav-text"><strong>Local workspace</strong><small>Your files never leave Chrome</small></div></div><div className="profile"><div className="avatar">IF</div><div className="nav-text"><strong>Free workspace</strong><small>Local only · No account</small></div><MoreHorizontal className="nav-text" size={17}/></div></div>
    </aside>
